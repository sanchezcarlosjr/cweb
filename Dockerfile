...
# and `pip install cWeb` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "cWeb", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n cWeb -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "cWeb", "/bin/bash", "-c"]
...
ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "cWeb", "cWeb web"]
